...
public class CalcularPromediosUseCase {
    private final static Logger log = Logger.getLogger(String.valueOf(CalcularPromediosUseCase.class));
    private static List<Persona> personas = null;

    public CalcularPromediosUseCase() {
    ...
    public void calcularPromedioPersonas() {
        int promedioSueldos = 0;

        for (Persona persona: personas) {
            promedioSueldos = promedioSueldos + persona.getSueldo();
        }

        log.info("El promedio de sueldos es de: " + promedioSueldos);
    }
}
